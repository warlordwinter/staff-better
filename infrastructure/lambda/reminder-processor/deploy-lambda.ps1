...
Write-Host "   Function name: ReminderProcessor" -ForegroundColor Gray
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        aws lambda get-function --function-name ReminderProcessor --region us-east-1 | Out-Null
        
        if ($LASTEXITCODE -eq 0) {
            Write-Host "✓ Deployment verified" -ForegroundColor Green
            Write-Host "`n💡 Next steps:" -ForegroundColor Cyan
            Write-Host "   1. Check CloudWatch Logs: /aws/lambda/ReminderProcessor" -ForegroundColor Gray
            Write-Host "   2. Test the function using the test command in SETUP_CHECKLIST.md" -ForegroundColor Gray
        }
    } else {
