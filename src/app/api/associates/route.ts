...
import { getAssociates, insertAssociates } from '@/lib/dao/AssociatesDao';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const insertedAssociates = await insertAssociates(associatesToInsert);
    return NextResponse.json(insertedAssociates, { status: 201 });
  } catch (error) {
    console.error('Failed to create associate:', error);
    return NextResponse.json({ error: 'Failed to create associate' }, { status: 500 });
  }
}