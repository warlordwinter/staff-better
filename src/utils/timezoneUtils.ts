...
function normalizeDateInput(date: string | Date): { y: number; m: number; d: number } {
 ...
 */
export function convertUTCTimeToLocal(utcTime: string, workDate: string | Date): string {
 ...
 */
export function convertLocalTimeToUTC(localTime: string | undefined, workDate: string | Date): string {
 ...
 */
export function formatTimeForDisplay(utcTime: string, workDate: string | Date): string {
  if (!utcTime || !workDate) return "";
  const [uh, um] = utcTime.split(":").map(Number);
  const { y, m, d } = normalizeDateInput(workDate);
  const ts = new Date(Date.UTC(y, m - 1, d, uh, um, 0));
  return new Intl.DateTimeFormat(undefined, { hour: "numeric", minute: "2-digit", hour12: true }).format(ts);
}
