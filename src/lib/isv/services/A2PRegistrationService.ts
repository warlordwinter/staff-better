...
export class A2PRegistrationService {
  ...
  async registerBrand(
    ...
    const twilioClient = await this.subaccountService.getCustomerTwilioClient(
      customerId
    );

    try {
      // Register brand via Twilio Messaging API
      // Note: Actual API endpoint may be different - should use TrustHub API
      // This is a placeholder implementation - proper implementation requires TrustHub API
      const messagingV1 = twilioClient.messaging.v1 as any;
      const brand = await messagingV1.brands.create({
  ...
  async registerCampaign(
    ...
    try {
      ...
      // This is a placeholder implementation - proper implementation requires TrustHub API
      const messagingV1 = twilioClient.messaging.v1 as any;
      const campaign = await messagingV1.campaigns.create({
  ...
  async getBrandStatus(brandId: string): Promise<Brand> {
    ...
    try {
      ...
      // Note: Actual API endpoint may be different - should use TrustHub API
      const messagingV1 = twilioClient.messaging.v1 as any;
      const twilioBrand = await messagingV1
        .brands(brand.twilio_brand_sid)
        .fetch();
  ...
  async getCampaignStatus(campaignId: string): Promise<Campaign> {
    ...
    try {
      ...
      // Note: Actual API endpoint may be different - should use TrustHub API
      const messagingV1 = twilioClient.messaging.v1 as any;
      const twilioCampaign = await messagingV1
        .campaigns(campaign.campaign_sid)
        .fetch();
