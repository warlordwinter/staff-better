  ...
  return (
    <div className="relative flex justify-between items-center">
      ...
      <div className="relative">
          ...
          <div className="absolute right-0 mt-2 p-3 bg-white border border-gray-200 rounded-lg shadow-lg z-20 min-w-64">
            ...
            <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
              <div 
                ...
                style={{ 
                  width: progress.step === 'idle' ? '0%' :
                         progress.step === 'parsing' ? '20%' :
                         progress.step === 'mapping' ? '40%' :
                         progress.step === 'transforming' ? '60%' :
                         progress.step === 'uploading' ? '80%' :
                         progress.step === 'complete' ? '100%' :
                         progress.step === 'error' ? '100%' : '0%'
                }}
              />
            </div>
          </div>
        ...
        {(progress.step === 'complete' || progress.step === 'error') && !isUploading && (
          <div className={`absolute right-0 mt-2 p-3 border rounded-lg shadow-lg z-20 min-w-64 ${
            progress.step === 'complete' ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'
          }`}>
            <div className="flex items-center justify-between mb-1">
              <h4 className={`text-sm font-medium ${
                progress.step === 'complete' ? 'text-green-900' : 'text-red-900'
              }`}>
                {progress.step === 'complete' ? 'Upload Complete!' : 'Upload Failed'}
              </h4>
              <button
                onClick={handleResetUpload}
                className="text-gray-400 hover:text-gray-600"
            ...
            <div className={`text-xs ${
              progress.step === 'complete' ? 'text-green-700' : 'text-red-700'
            }`}>
              {progress.message}
            </div>
          </div>
